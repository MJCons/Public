...
class Critic(nn.Module):
    def __init__(self, n_agent, dim_states, dim_action):
        super(Critic, self).__init__()
        self.n_agent = n_agent
        self.dim_observation = dim_states
    ...
    def forward(self, obs, acts):
        ...
        # print("result和acts的shape：", result.shape, acts.shape)
        combined = th.cat([result, acts], 1)
        result = F.relu(self.FC2(combined))
        # print("combined的shape为：", combined.shape)
